...
import os
# 1. 先加入路径
sys.path.append("/home/ma-user/work/lbx/lvr-main/")
# os.environ["ASCEND_RT_VISIBLE_DEVICES"] = "0"


# import time  # 添加time模块导入
# # # 1. 先sleep 5小时
# print(f"开始休眠2小时... 当前时间: {time.strftime('%Y-%m-%d %H:%M:%S')}", flush=True)
# time.sleep(2*60*60)  # 5小时 = 5 * 60分钟 * 60秒
# print(f"休眠结束，开始执行主程序... 当前时间: {time.strftime('%Y-%m-%d %H:%M:%S')}", flush=True)
...
def load_vstar_dataset(gen_w_head,run_name,decoding_strategy):
    ...
    ds = load_dataset("/home/ma-user/work/lbx/dataset/evaluation/vstar_bench")
    image_dir = "/home/ma-user/work/lbx/dataset/evaluation/vstar_bench"

    if gen_w_head:
        out_dir = os.path.join("/home/ma-user/work/lbx/lvr-main/evaluation/vstar_bench/results",f"decoding_by_{decoding_strategy}","GenWHead"+run_name)

    else:
        out_dir = os.path.join("/home/ma-user/work/lbx/lvr-main/evaluation/vstar_bench/results",f"decoding_by_{decoding_strategy}",run_name)
...
def load_mmvp_dataset(gen_w_head,run_name,decoding_strategy):
    ds_name = "mmvp"
    csv_file = "/home/ma-user/work/lbx/dataset/evaluation/MMVP/Questions.csv"
    image_dir = "/home/ma-user/work/lbx/dataset/evaluation/MMVP/MMVP Images"
    ...
    if gen_w_head:
        out_dir = os.path.join("/home/ma-user/work/lbx/lvr-main/evaluation/MMVP/results",f"decoding_by_{decoding_strategy}","GenWHead"+run_name)

    else:
        out_dir = os.path.join("/home/ma-user/work/lbx/lvr-main/evaluation/MMVP/results",f"decoding_by_{decoding_strategy}",run_name)
...
def load_blink_dataset(gen_w_head,run_name,decoding_strategy):
    ...
    for config in configs:
        ...
        all_datasets[config] = load_dataset("/home/ma-user/work/lbx/dataset/evaluation/Blink", config)
    ...
    if gen_w_head:
        out_dir = os.path.join("/home/ma-user/work/lbx/lvr-main/evaluation/Blink/results",f"decoding_by_{decoding_strategy}","GenWHead"+run_name)

    else:
        out_dir = os.path.join("/home/ma-user/work/lbx/lvr-main/evaluation/Blink/results",f"decoding_by_{decoding_strategy}",run_name)
